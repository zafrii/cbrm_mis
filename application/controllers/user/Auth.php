...
class Auth extends MY_Controller {
	...
	public function test_email(){
		...
		$mail_data = array(
			'fullname' => 'Muhammad Muzaffar',
			'verification_link' => base_url('user/auth/verify/').'/437d7d1d97917cd627a34a6a0fb41136'
		);

		$to = 'h_zafri_gis@yahoo.com';

		$email = $this->mailer->mail_template($to,'email-verification',$mail_data);

		if($email){
			echo 'Your Account has been made, please verify it by clicking the activation link that has been send to your email.';	
	...
	public function login(){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if($result){
					...
					if(1 /*$result['is_admin'] == 1*/){
						...
						// 	redirect(base_url('user/agreement'), 'refresh');
						// }
						redirect(base_url('user/dashboard/index'), 'refresh');
				...
				$this->load->view('user/includes/_footer', $data);
			}
		}	
		...
		public function register(){
			...
			if($this->input->post('submit')){
				...
				$this->form_validation->set_rules('mobile_no', 'Mobilenumber', 'trim|required');
				...
				if ($this->form_validation->run() == FALSE) {
					...
					redirect(base_url('user/auth/register'),'refresh');
				}
				else{
					$data = array(
						'username' => $this->input->post('username'),
					...
					if($result){
						...
						$mail_data = array(
							...
							'verification_link' => base_url('user/auth/verify/').'/'.$data['token']
						);
		...
		public function forgot_password(){
			...
			if($this->input->post('submit')){
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$this->session->set_flashdata('errors', $data['errors']);
					redirect(base_url('user/auth/forget_password'),'refresh');
				}
				...
				if($response){
					...
					$mail_data= array(
						...
						'reset_link' => base_url('user/auth/reset_password/'.$pwd_reset_code)
					);
					...
					if($email){
						...
						redirect(base_url('user/auth/forgot_password'));
					}
					else{
				...
				$this->load->view('user/includes/_header', $data);
				$this->load->view('user/auth/forget_password');
				$this->load->view('user/includes/_footer', $data);
			}
		}
		...
		public function reset_password($id=0){
			...
			if($this->input->post('submit')){
				...
				if($result){
					...
					$this->load->view('user/includes/_header', $data);
					$this->load->view('user/auth/reset_password');
					$this->load->view('user/includes/_footer', $data);
